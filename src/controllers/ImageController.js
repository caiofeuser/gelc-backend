...
module.exports = {
	...
	async index(req,res) {
		...
		try {
			...
			const docs = await Image.paginate({},{page,select:'-to -toModel',limit:10});
	...
	async store(req,res) {
		...
		switch(tomodel) {
			...
			case 'project':
				...
				let {coordinator,secondCoordinator} = await mongoose.model('Project').findById(to).select('coordinator secondCoordinator');

				if(authParticipantId!=coordinator && authParticipantId!=secondCoordinator && authParticipantPermission!=='master') {
			...
			case 'post':
				...
				let {author} = await mongoose.model('Post').findById(to).select('author');
				if(authParticipantId!=author && authParticipantPermission!=='master') {
					fs.unlinkSync(req.file.path);
					return res.status(403).send({message:'forbidden'});
		...
		try {
			...
			let fileData = await sharp(req.file.path).resize(resolution).jpeg({quality:65}).toBuffer();
		...
		try {
			...
			switch (doc.toModel) {
				...
				case 'Participant':
					...
					if (authParticipantId!=doc.to._id && authParticipantPermission!=='master') {
				...
				case 'Project':
					...
					if (authParticipantId!=doc.to.coordinator && authParticipantId!=doc.to.secondCoordinator && authParticipantPermission!=='master') {
				...
				case 'Post':
					...
					if (authParticipantId!=doc.to.author && authParticipantPermission!=='master') {
