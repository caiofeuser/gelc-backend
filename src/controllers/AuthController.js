...
module.exports = {
	...
	async login(req,res) {
		...
		try {
			...
			let participant = await Participant.findOne({email}).select('+password +permission');
			...
			if(await bcrypt.compare(password, participant.password)) {
				...
				let token = jwt.sign({_id, permission}, config.jwt.SECRET,{expiresIn:config.jwt.TOKEN_EXP_TIME});
			...
			return res.status(500).send({message: 'a problem was encountered on the server'});
	...
	async resetPassword(req,res) {
		...
		try {
			...
			let participant = await Participant.findOne({email}).select('+passwordResetToken +passwordResetExpires');
			...
			return res.status(500).send({message:'cannot reset password, try again'});
